				...
				<div class="col-lg-offset-2 col-lg-7">
					<form class="form-horizontal" method="post" action="" id="edit_group_personnel">
						...
						<div class="form-group">
							...
							<div class="col-sm-8">
								<input type="text" class="form-control" name="title" value="{{old('title',isset($data[$id]['title']) ? $data[$id]['title']: null )}}" required>
							</div>
						</div>
						<div class="form-group">
								        ...
								        <tbody class="detail">
								            @foreach ($listPersonnel as $key=>$value)
												<tr class="{{ ($value['date_out'] != '') ? 'un-working' : 'working' }}">
													...
													<td>
														<input type="checkbox" name="personnel_id[{{ $value['id'] }}]" value="{{ $value['id'] }}" <?php if(  isset($listPersonnel[$key]['ticket']) && $listPersonnel[$key]['ticket']==1 ){ echo "checked"; } ?>>
													</td>
												</tr>
								            @endforeach
			...
			<script type="text/javascript">
				$(document).ready(function() {
					$('#edit_group_personnel').submit(function(event) {
						var link = "{{ route('editGroupPersonalConfigAjax') }}";
						var selected = [];
