				...
				<h4 class="title-fuction">
					...
					<div class="pull-right" >
						@if( empty($data) || $data[0]->status ==1)
							<button type="button" class="btn btn-sm btn-orange special salary"><img src="{{ asset('images/general/calculator.png') }}"></button>
						@endif
					</div>
				</h4>
				    ...
				    @if(!empty($data))
				    		...
				    		?>
					     	@foreach ($data as $val)
					    		...
					    		?>
					     		<!-- Chính thức -->
					     		@if( $val->salary_official_work > 0 && $val->salary_trial_work == 0 && $val->salary_parttime_work == 0 && $val->salary_trainee_work == 0 && $val->salary_trainee_parttime_work == 0 )
                                ...
                                @elseif( $val->salary_trainee_parttime_work > 0 && $val->salary_trial_work == 0 && $val->salary_official_work == 0 && $val->salary_parttime_work == 0 && $val->salary_trainee_work == 0)
								    <tr style="background: rgba(240, 173, 78, 0.34);">
								      	<td class="text-nowrap" scope="row">{{ str_limit( $val->fullname, $limit = 35, $end = '...') }}</td> 
										<td>{{ BatvHelper::formatPrice($val->salary_trainee_parttime_work) }}</td>
										<td>{{ BatvHelper::formatPrice($val->salary_overtime) }}</td>
										<td><?php  if( $val->money_work_late > 0 ) echo "-"; ?>{{ BatvHelper::formatPrice($val->money_work_late) }}</td>
										<td>{{ BatvHelper::formatPrice($val->mulct_money_awol ) }}</td>
										<td>{{ BatvHelper::formatPrice($val->salary_overtime + $val->salary_trainee_parttime_work-$val->money_work_late+$val->mulct_money_awol) }}</td>
								    </tr>
							    <!-- Nửa này nửa kia -->
							    @else
								    	...
								    	@if( $val->salary_parttime_work >0 && ( $val->salary_official_work >0 || $val->salary_trainee_parttime_work >0  )  )
										    ...
										    <tr style="background: #aadcaa;" >
												...
												<td rowspan="2" style="padding: 25px 0;"><?php  if( $val->money_work_late > 0 ) echo "-"; ?>{{ BatvHelper::formatPrice($val->money_work_late) }}</td>
												<td rowspan="2" style="padding: 25px 0;">{{ BatvHelper::formatPrice($val->mulct_money_awol ) }}</td>
												<td rowspan="2" style="padding: 25px 0;">{{ BatvHelper::formatPrice($val->salary_overtime + $val->salary_parttime_work+$work-$val->money_work_late+$val->mulct_money_awol) }}</td>
                                        ...
                                        @if( $val->salary_trainee_parttime_work >0 && $val->salary_official_work >0 )
										    <tr style="background: #aadcaa;" >
												...
												<td rowspan="2" style="padding: 25px 0;"><?php  if( $val->money_work_late > 0 ) echo "-"; ?>{{ BatvHelper::formatPrice($val->money_work_late) }}</td>
												<td rowspan="2" style="padding: 25px 0;">{{ BatvHelper::formatPrice($val->mulct_money_awol ) }}</td>
												<td rowspan="2" style="padding: 25px 0;">{{ BatvHelper::formatPrice($val->salary_overtime + $val->salary_trainee_parttime_work+$val->salary_official_work-$val->money_work_late+$val->mulct_money_awol) }}</td>
					    	...
					    	@endforeach
					    					<tr style="background: rgba(255, 0, 0, 0.56);">
					    						<td><b>TỔNG HỢP</b></td>
					    						<td><b>{{ BatvHelper::formatPrice( $total_salary_official_work + $total_salary_trial_work + $total_salary_parttime_work + $total_salary_trainee_work + $total_salary_trainee_parttime_work  ) }}</b></td>
					    						...
					    						<td><b>{{ BatvHelper::formatPrice($total_salary_overtime + $total_salary_official_work + $total_salary_trial_work + $total_salary_parttime_work + $total_salary_trainee_work + $total_salary_trainee_parttime_work -$total_money_work_late+$total_mulct_money_awol) }}</b></td>
					    					</tr>
					    @endif
				        </tbody>
				    </table>
