	                    ...
	                    <div class="form-group col-lg-5">
					 	...
					 	<div class="form-group col-lg-12">
				          <div>
				            ...
				            {{-- <input type="submit" class="btn btn-sm btn-orange" value="Gửi Email" name="send_email" onclick="return confirm('Bạn thực sự muốn gửi Email ?')"> --}}
							@if( empty($data) || $data[0]->status_bonus ==1)
							...
							@else 
								@if(in_array('luongthuong-tinhlailuong',$arr_route))
					    ...
					    @if(!empty($data))
				    		...
				    		?>
					     	@foreach ($data as $val)
								...
								?>
					     		<!-- Chính thức -->
					     		@if( ($val->holiday_bonus >0 || $val->salary_official_work > 0 || $total_item > 0) && $val->salary_trial_work == 0 && $val->salary_parttime_work == 0 && $val->salary_trainee_work == 0 && $val->salary_trainee_parttime_work == 0 )
					     			
								    <tr style="background: #f5f5f5;">
										<td style="min-width: 210px; text-align: left;">
										  <span style="padding-right: 10px;">{{ $stt }}.</span> {{ str_limit( $val->fullname, $limit = 35, $end = '...') }} 
										</td> 
										<td>{{ BatvHelper::formatPrice($val->salary_official_default) }}</td>
										<td>{{ BatvHelper::getInfoRatioInSalary($dateCurrent,$val->personnel_id) }}</td>
										<td>-</td>
										...
										<td>{{ BatvHelper::formatPrice($val->mulct_money_awol) }}</td><!-- Trừ lương khi vượt số ngày phép trong tháng -->
										<td>{{ BatvHelper::formatPrice($val->holiday_bonus) }}</td><!-- Thưởng ngày lễ -->
										<td>
											{{ BatvHelper::formatPrice($total_item) }} <!-- các khoản khác -->
										</td>
										<td>{{ BatvHelper::formatPrice($val->salary_overtime) }}</td><!-- Lương làm thêm giờ -->
										<td>
										...
										<td>{{ BatvHelper::formatPrice($val->insurance_by_company + $val->insurance) }}</td><!--  Tổng bảo hiểm -->
										<td class="item-dn">
											<?php
												$flag_tmp = $val->salary_overtime + $val->salary_official_work+$val->management_allowance+ $val->lunch_allowance+ $val->travel_allowance+$val->phone_allowance+$val->movement_allowance+$val->work_bonus-$val->money_work_late+$val->parking_fee_allowance+$total_item+$val->other_tax_allowance+$val->laptop_allowance+$val->mulct_money_awol + $val->holiday_bonus - $val->party_fee;
								    ...
								    <?php  $stt++; ?>
							    <!-- Thử việc -->
							    @elseif( $val->salary_trial_work> 0 && $val->salary_official_work  == 0 && $val->salary_parttime_work == 0 && $val->salary_trainee_work == 0 && $val->salary_trainee_parttime_work == 0)
								     <tr style="background: #5bc0de;" >
										...
										<td>{{ BatvHelper::formatPrice($val->holiday_bonus) }}</td><!-- Thưởng ngày lễ -->
										<td>{{ BatvHelper::formatPrice($total_item) }}</td> <!-- các khoản khác -->
										<td>{{ BatvHelper::formatPrice($val->salary_overtime) }}</td><!-- Lương làm thêm giờ -->
										<td></td><!-- Trả nợ định kỳ kỳ vay vốn -->
										<td>0</td><!-- Lương lần 1 -->
										<td>
											{{ BatvHelper::formatPrice( $val->salary_overtime + $val->salary_trial_work+$val->management_allowance+$val->work_bonus-$val->money_work_late+$val->parking_fee_allowance+$val->mulct_money_awol-$val->party_fee) }}
										...
										<td class="item-thucnhan">{{ BatvHelper::formatPrice( $val->salary_overtime + $val->salary_trial_work+$val->management_allowance+$val->work_bonus-$val->money_work_late+$val->parking_fee_allowance+$total_item+$val->mulct_money_awol+ $val->holiday_bonus-$val->party_fee) }}</td><!-- Thực nhận -->
										<td>{{ BatvHelper::formatPrice($val->insurance_by_company + $val->insurance) }}</td><!--  Tổng bảo hiểm -->
										<td class="item-dn">{{ BatvHelper::formatPrice( $val->salary_overtime + $val->salary_trial_work+$val->management_allowance+$val->work_bonus-$val->money_work_late+$val->parking_fee_allowance +$total_item+$val->mulct_money_awol+ $val->holiday_bonus-$val->party_fee) }}</td><!-- Doanh nghiệp phải chi trả -->
								    </tr>
								    <?php  $stt++; ?>
					     		<!-- Parttime -->
					     		@elseif( $val->salary_parttime_work > 0 && $val->salary_trial_work == 0 && $val->salary_official_work == 0 && $val->salary_trainee_work == 0 && $val->salary_trainee_parttime_work == 0)
								    <tr style="background: rgb(147, 147, 193);">
										...
										<td>0</td><!-- Lương lần 1 -->
										<td>
											{{ BatvHelper::formatPrice( $val->salary_overtime + $val->salary_parttime_work+$val->management_allowance+ $val->lunch_allowance+ $val->travel_allowance+$val->phone_allowance+$val->movement_allowance+$val->work_bonus-$val->insurance-$val->money_work_late-$val->welfare_fund+$val->parking_fee_allowance+$val->other_tax_allowance+$val->laptop_allowance+$val->mulct_money_awol-$val->party_fee) }}
										...
										<td class="item-thucnhan">{{ BatvHelper::formatPrice( $val->salary_overtime + $val->salary_parttime_work+$val->management_allowance+ $val->lunch_allowance+ $val->travel_allowance+$val->phone_allowance+$val->movement_allowance+$val->work_bonus-$val->insurance-$val->money_work_late-$val->welfare_fund+$val->parking_fee_allowance+$total_item+$val->other_tax_allowance+$val->laptop_allowance+$val->mulct_money_awol+$val->holiday_bonus-$val->party_fee) }}</td><!-- Thực nhận -->
										<td>{{ BatvHelper::formatPrice($val->insurance_by_company + $val->insurance) }}</td><!--  Tổng bảo hiểm -->
										<td class="item-dn">{{ BatvHelper::formatPrice($val->salary_overtime +  $val->salary_parttime_work+$val->management_allowance+ $val->lunch_allowance+ $val->travel_allowance+$val->phone_allowance+$val->movement_allowance+$val->work_bonus+$val->insurance_by_company-$val->money_work_late+$val->parking_fee_allowance +$total_item+$val->other_tax_allowance+$val->laptop_allowance+$val->mulct_money_awol+$val->holiday_bonus-$val->party_fee) }}</td><!-- Doanh nghiệp phải chi trả -->
								    </tr>
								    <?php  $stt++; ?>
					     		<!-- Thực tập fulltime -->
					     		@elseif( $val->salary_trainee_work > 0 && $val->salary_trial_work == 0 && $val->salary_official_work == 0 && $val->salary_parttime_work == 0 && $val->salary_trainee_parttime_work == 0)
								    <tr style="background: rgba(240, 173, 78, 0.66);">
										...
										<td>0</td><!-- Lương lần 1 -->
										<td>
											{{ BatvHelper::formatPrice( $val->salary_overtime + $val->salary_trainee_work+$val->management_allowance+ $val->lunch_allowance+ $val->travel_allowance+$val->phone_allowance+$val->movement_allowance+$val->work_bonus-$val->insurance-$val->money_work_late-$val->welfare_fund+$val->parking_fee_allowance+$val->other_tax_allowance+$val->laptop_allowance+$val->mulct_money_awol-$val->party_fee) }}
											<?php  $total_salary_2 +=  round($val->salary_overtime + $val->salary_trainee_work+$val->management_allowance+ $val->lunch_allowance+ $val->travel_allowance+$val->phone_allowance+$val->movement_allowance+$val->work_bonus-$val->insurance-$val->money_work_late-$val->welfare_fund+$val->parking_fee_allowance+$val->other_tax_allowance+$val->laptop_allowance+$val->mulct_money_awol-$val->party_fee); ?>
										</td>
										<td class="item-thucnhan">{{ BatvHelper::formatPrice( $val->salary_overtime + $val->salary_trainee_work+$val->management_allowance+ $val->lunch_allowance+ $val->travel_allowance+$val->phone_allowance+$val->movement_allowance+$val->work_bonus-$val->insurance-$val->money_work_late-$val->welfare_fund+$val->parking_fee_allowance+$total_item+$val->other_tax_allowance+$val->laptop_allowance+$val->mulct_money_awol+$val->holiday_bonus-$val->party_fee) }}</td><!-- Thực nhận -->
										<td>{{ BatvHelper::formatPrice($val->insurance_by_company + $val->insurance) }}</td><!--  Tổng bảo hiểm -->
										<td class="item-dn">{{ BatvHelper::formatPrice($val->salary_overtime +  $val->salary_trainee_work+$val->management_allowance+ $val->lunch_allowance+ $val->travel_allowance+$val->phone_allowance+$val->movement_allowance+$val->work_bonus+$val->insurance_by_company-$val->money_work_late+$val->parking_fee_allowance +$total_item+$val->other_tax_allowance+$val->laptop_allowance+$val->mulct_money_awol+$val->holiday_bonus-$val->party_fee) }}</td><!-- Doanh nghiệp phải chi trả -->
								    </tr>
								    <?php  $stt++; ?>
                                <!-- Thực tập parttime -->
                                @elseif( $val->salary_trainee_parttime_work > 0 && $val->salary_trial_work == 0 && $val->salary_official_work == 0 && $val->salary_parttime_work == 0 && $val->salary_trainee_work == 0)
                                    <tr style="background: rgba(240, 173, 78, 0.34);">
                                       ...
                                       <td>  
										...
										<td>0</td><!-- Lương lần 1 -->
										<td>
											{{ BatvHelper::formatPrice( $val->salary_overtime + $val->salary_trainee_parttime_work+$val->management_allowance+ $val->lunch_allowance+ $val->travel_allowance+$val->phone_allowance+$val->movement_allowance+$val->work_bonus-$val->insurance-$val->money_work_late-$val->welfare_fund+$val->parking_fee_allowance+$val->other_tax_allowance+$val->laptop_allowance+$val->mulct_money_awol-$val->party_fee) }}
                                        ...
                                        <td class="item-thucnhan">{{ BatvHelper::formatPrice( $val->salary_overtime + $val->salary_trainee_parttime_work+$val->management_allowance+ $val->lunch_allowance+ $val->travel_allowance+$val->phone_allowance+$val->movement_allowance+$val->work_bonus-$val->insurance-$val->money_work_late-$val->welfare_fund+$val->parking_fee_allowance+$total_item+$val->other_tax_allowance+$val->laptop_allowance+$val->mulct_money_awol+$val->holiday_bonus-$val->party_fee) }}</td><!-- Thực nhận -->
										<td>{{ BatvHelper::formatPrice($val->insurance_by_company + $val->insurance) }}</td><!--  Tổng bảo hiểm -->
                                        <td class="item-dn">{{ BatvHelper::formatPrice( $val->salary_overtime + $val->salary_trainee_parttime_work+$val->management_allowance+ $val->lunch_allowance+ $val->travel_allowance+$val->phone_allowance+$val->movement_allowance+$val->work_bonus+$val->insurance_by_company-$val->money_work_late+$val->parking_fee_allowance +$total_item+$val->other_tax_allowance+$val->laptop_allowance+$val->mulct_money_awol+$val->holiday_bonus-$val->party_fee) }}</td><!-- Doanh nghiệp phải chi trả -->
                                    </tr>
								    <!-- Nửa này nửa kia -->
								    <?php  $stt++; ?>
							    @else
								    	...
								    	@if( $val->salary_trainee_work >0 && ( $val->salary_official_work >0 || $val->salary_trainee_parttime_work >0  || $val->salary_parttime_work >0 ) )
											...
											?>
										     <tr style="background: #aadcaa;" >
												...
												<td class="item-thucnhan">{{ BatvHelper::formatPrice( $val->salary_trainee_work +$val->holiday_bonus-$val->party_fee) }}</td><!-- Thực nhận -->
												<td rowspan="2" style="position: relative;top: 15px;">0</td><!-- Tổng bảo hiểm -->
												<td  rowspan="2" style="position: relative;top: 15px;"  class="item-dn">{{ BatvHelper::formatPrice( $val->salary_trainee_work+$work+$val->management_allowance+ $val->lunch_allowance+ $val->travel_allowance+$val->phone_allowance+$val->movement_allowance+$val->work_bonus+$val->insurance_by_company-$val->money_work_late+$total_item+$val->other_tax_allowance+$val->laptop_allowance +$val->mulct_money_awol +$val->parking_fee_allowance+$val->holiday_bonus-$val->party_fee) }}</td><!-- Doanh nghiệp phải chi trả -->
								    	...
								    	@if( $val->salary_parttime_work >0 && ( $val->salary_official_work >0 || $val->salary_trainee_parttime_work >0  )  )
											...
											?>
										     <tr style="background: #aadcaa;" >
												...
												<td>0</td><!-- Lương lần 1 -->
												<td rowspan="2" style="position: relative;top: 15px;">
														{{ BatvHelper::formatPrice( $val->salary_parttime_work+$work+$val->management_allowance+ $val->lunch_allowance+ $val->travel_allowance+$val->phone_allowance+$val->movement_allowance+$val->work_bonus+$val->insurance_by_company-$val->money_work_late+$val->other_tax_allowance+$val->laptop_allowance +$val->mulct_money_awol+$val->parking_fee_allowance-$val->party_fee) }}
														<?php  
												...
												<td  class="item-thucnhan">{{ BatvHelper::formatPrice( $val->salary_parttime_work +$val->holiday_bonus-$val->party_fee) }}</td><!-- Thực nhận -->
												<td rowspan="2" style="position: relative;top: 15px;">0</td><!-- Tổng bảo hiểm -->
												<td rowspan="2" style="position: relative;top: 15px;"  class="item-dn">{{ BatvHelper::formatPrice( $val->salary_parttime_work+$work+$val->management_allowance+ $val->lunch_allowance+ $val->travel_allowance+$val->phone_allowance+$val->movement_allowance+$val->work_bonus+$val->insurance_by_company-$val->money_work_late+$total_item+$val->other_tax_allowance+$val->laptop_allowance +$val->mulct_money_awol+$val->parking_fee_allowance+$val->holiday_bonus-$val->party_fee) }}</td><!-- Doanh nghiệp phải chi trả -->
                                        ...
                                        @if( $val->salary_trainee_parttime_work >0 && $val->salary_official_work >0 )
                                        	...
                                        	?>
                                             <tr style="background: #aadcaa;" >
												...
												<td>0</td><!-- Lương lần 1 -->
												<td rowspan="2" style="position: relative;top: 15px;">
													{{ BatvHelper::formatPrice( $val->salary_trainee_parttime_work+$salary_official_work+$val->management_allowance+ $val->lunch_allowance+ $val->travel_allowance+$val->phone_allowance+$val->movement_allowance+$val->work_bonus+$val->insurance_by_company-$val->money_work_late+$val->other_tax_allowance+$val->laptop_allowance +$val->mulct_money_awol+$val->parking_fee_allowance-$val->party_fee) }}
													<?php  
                                                ...
                                                <td class="item-thucnhan">{{ BatvHelper::formatPrice( $val->salary_trainee_parttime_work +$val->holiday_bonus-$val->party_fee) }}</td><!-- Thực nhận -->
                                                <td rowspan="2" style="position: relative;top: 15px;">0</td><!-- Tổng bảo hiểm -->
                                                <td rowspan="2" style="position: relative;top: 15px;"  class="item-dn">{{ BatvHelper::formatPrice( $val->salary_trainee_parttime_work+$salary_official_work+$val->management_allowance+ $val->lunch_allowance+ $val->travel_allowance+$val->phone_allowance+$val->movement_allowance+$val->work_bonus+$val->insurance_by_company-$val->money_work_late+$total_item+$val->other_tax_allowance+$val->laptop_allowance +$val->mulct_money_awol+$val->parking_fee_allowance+$val->holiday_bonus-$val->party_fee) }}</td><!-- Doanh nghiệp phải chi trả -->
					    	...
					    	@endforeach
					    					<tr style="background: rgba(255, 0, 0, 0.56);">
					    						<td><b>TỔNG HỢP</b></td>
					    						<td><b>{{ BatvHelper::formatPrice($total_salary_official_default+$total_salary_trial_default_special+$total_salary_trainee_parttime_default+$total_salary_trainee_default+$total_salary_parttime_default) }}</b></td>
					    						<td></td>
					    						<td><b>{{ BatvHelper::formatPrice($total_salary_trial_default) }}</b></td>
					    						<td><b></b></td>
					    						<td><b></b></td>
					    						<td><b></b></td>
					    						<td><b></b></td>
					    						<td><b>{{ BatvHelper::formatPrice($total_salary_official_work+$total_salary_trainee_parttime_work+$total_salary_trainee_work+$total_salary_parttime_work) }}</b></td>
												<td><b>{{ BatvHelper::formatPrice($total_salary_trial_work) }}</b></td>
												...
												<td><b>{{ BatvHelper::formatPrice($total_money_work_late) }}</b></td>
					    						<td><b>{{ BatvHelper::formatPrice($total_welfare_fund) }}</b></td>
					    						<td><b>{{ BatvHelper::formatPrice($total_parking_fee_allowance) }}</b></td>
												<td><b>{{ BatvHelper::formatPrice($total_party_fee) }}</b></td>
					    						<td><b>{{ BatvHelper::formatPrice($total_mulct_money_awol) }}</b></td>
					    						<td><b>{{ BatvHelper::formatPrice($total_holiday_bonus) }}</b></td>
					    						<td><b>{{ BatvHelper::formatPrice($total) }}</b></td>
												<td><b>{{ BatvHelper::formatPrice($total_salary_overtime) }}</b></td>
												<td><b>{{ BatvHelper::formatPrice($total_pay_month_loan_capital) }}</b></td>
												<td><b>{{ BatvHelper::formatPrice($total_salary_1) }}</b></td>
												<td><b>{{ BatvHelper::formatPrice($total_salary_2) }}</b></td>
					    						<td><b class="sum-thucnhan"></b></td>
					    						<td><b>{{ BatvHelper::formatPrice($all_insurance) }}</b></td>
			...
			<script type="text/javascript">
				...
				$('body').on('click','.done_status_bonus',function(){
					if (confirm('Bạn có chắc chắn muốn Chốt thưởng?')) {
						var link = "{{ route('getSalaryBonusDoneAjax') }}";
						var data = {
				...
				$('body').on('click','.done',function(){
					if (confirm('Bạn có chắc chắn muốn Chốt lương tháng này?')) {
						var link = "{{ route('getSalaryDoneAjax') }}";
						var data = {
								selectMonth:<?php echo isset($_GET['selectMonth'])?$_GET['selectMonth']:date('m') ?>,
								selectYear:<?php echo isset($_GET['selectYear'])?$_GET['selectYear']:date('Y') ?>,
							};
				...
				$('body').on('click','.r-done',function(){
					if (confirm('Lưu ý: Nếu đã trả lương cho nhân viên rồi mà tính lại có thể sẽ có sự thay đổi. Bạn vẫn muốn tiếp tục ?')) {
						var link = "{{ route('getSalaryRecalCulationAjax') }}";
						var data = {
								selectMonth:<?php echo isset($_GET['selectMonth'])?$_GET['selectMonth']:date('m') ?>,
								selectYear:<?php echo isset($_GET['selectYear'])?$_GET['selectYear']:date('Y') ?>,
							};
