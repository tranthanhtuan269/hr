                            ...
                            @if(!empty($data))
                                ...
                                @foreach ($data as $key=>$val)
                                 <tr>
                                    ...
                                    <td>{{ $val['hsl_old'] }}</td>
                                    <td>{{ $val['hsl_ht'] }}</td>
                                    <td>{{ BatvHelper::formatDate($val["date_nlgn"],"Y-m-d", $formatDate="d/m/Y",$timeFormat="H:i:s",$time=false) }}</td>
                                    <td>{{ $val["period"]["from"] }}-{{ $val["period"]["to"] }}</td>
                                    <td>{{ round($val["param_hs"],3) }}</td>
<!--                                     <td>{{ $val['number_tt']['month'] }} tháng @if( $val['number_tt']['days'] > 0 )  {{ $val['number_tt']['days'] }} ngày @endif</td> -->
                                    <td><span class="col-sm-6" style="margin-right: 5px;">{{ BatvHelper::formatPrice( $val['value_tt'] ) }} </span>&nbsp
                                        @if($val['type']!=1)
                                            <a href="#" class="btn btn-xs btn-orange text-right" data-toggle="modal" data-target="#editSalaryTL{{ $val['personnel_id'] }}" style="text-decoration: none;">Sửa</a>
                                            <!-- POPUP -->
                                            <div id="editSalaryTL{{ $val['personnel_id']  }}" class="modal fade" role="dialog">
                                              <div class="modal-dialog">
                                                <div class="modal-content">
                                                    ...
                                                    <form class="form-horizontal" id="editSalaryTL{{ $val['personnel_id'] }}">
                                                        {!! csrf_field()!!}
                                                        <div class="modal-body row">
                                                            <div class="form-group clearfix">
                                                                <label class="col-sm-4" style="text-align: right;">Nhập số tiền: </label>
                                                                <div class="col-sm-7">
                                                                    <input type="text" onkeyup="format_curency_general(this.value,'numFormatResult{{ $val["personnel_id"] }}','result{{ $val["personnel_id"] }}');" id="numFormatResult{{ $val['personnel_id'] }}" class="form-control" value="{{ old('income_value') }}" >
                                                                    <input type="hidden" id="result{{ $val['personnel_id'] }}">
                                                                </div>
                                                            </div>
                                                        </div>
                                                        <div class="modal-footer">
                                                        
                                                            <div id="pre_ajax_loading_updated{{ $val['personnel_id'] }}" class="hide" style="text-align: center;margin-bottom: 10px;"><img src="{{ asset('images/general/bx_loader.gif') }}"></div>
                                                            <button type="button" class="btn btn-xs btn-orange" onclick="editSalaryTL({{ $val['personnel_id'] }})">Cập nhật</button>
                                                            <div class="ajax_response_updated {{ $val['personnel_id'] }}" style="display: none;margin: 10px 0px;padding: 6px 0px;font-size: 12px;"></div>
                                    ...
                                    <td style="width: 125px;">
                                        <input type="hidden" name="month{{ $val['personnel_id'] }}" value='{{ BatvHelper::formatDate($val["date_nlgn"],"Y-m-d", $formatDate="m",$timeFormat="H:i:s",$time=false) }}' >
                                        <input type="hidden" name="year{{ $val['personnel_id'] }}" value='{{ BatvHelper::formatDate($val["date_nlgn"],"Y-m-d", $formatDate="Y",$timeFormat="H:i:s",$time=false) }}' >
                                        <input type="hidden" name="income_value{{ $val['personnel_id'] }}" value='{{ $val["value_tt"]}}'>
                                        @if($val['type']==1)
                                            <span class="daduyet">Đã duyệt</span>
                                        @else
                                            <div id="pre_ajax_loading{{ $val['personnel_id'] }}" class="hide" style="text-align: center;margin-bottom: 10px;"><img src="{{ asset('images/general/bx_loader.gif') }}"></div>
                                            <button type="button" class="btn btn-xs btn-orange" onclick="updateData({{ $val['personnel_id'] }})">Phê duyệt</button>
                                            <div class="ajax_response {{ $val['personnel_id'] }}" style="display: none;margin: 10px 0px;padding: 6px 0px;font-size: 12px;"></div>
                                            <input type="hidden" id="income_value{{ $val['personnel_id'] }}" value="{{ BatvHelper::formatPrice( $val['value_tt'] ) }}">
...
<script type="text/javascript">
    ...
    function updateData(id){
        ...
        if (tmp == true) {
            ...
            $.ajax({
                method: "GET",
                url: "{{ route('approvalSalaryTLAjax')}}",
                data: param,
                beforeSend: function() {
