								        ...
								        <tbody class="special">
										     	@foreach($data as $v)
												     <tr class="message-box text-center" id="message_{{ $v->id }}">
														...
														<td class="message-value">{{ $v->value }}</td>
														<td class="message-salary">{{ $v->salary_basic }}</td>
														<td class="message-welfarefund">{{ $v->welfare_fund }}</td>
		...
		switch(action) {
			case "add":
				...
				if ( (!isNaN(value) && value>0) && (!isNaN(salary_basic) && salary_basic>0)  && (!isNaN(welfarefund) && welfarefund>0)){
					$.ajax({
					url: "{{ route('settingSalaryBasicAjax') }}",
					data:queryString,
					type: "GET",
