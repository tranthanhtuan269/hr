...
@section('content')
        ...
        <div class="col-lg-10">
            ...
            <div class="form-group col-lg-12">
                <div class="alert alert-warning">
                   Bạn có <strong id="count_reports_pending">{{ $number_report_pending }}</strong> báo cáo chưa duyệt
                </div>
                <label><input type="checkbox" name="show_reports_pending" @if(Request::get('show_reports_pending') == 1) checked @endif value="1"> Chỉ hiển thị các báo cáo chưa duyệt</label>
            </div>
            @if (count($list) > 0)
                <table class="table current" style="margin-bottom: 5px;">
                    ...
                    <tbody>
                        ...
                        @foreach ($list as $key => $value)
                            @foreach ($value->detail as $k_detail => $v_detail)
                                <tr>
                                    ...
                                    <td>
                                        {!! ($v_detail->day_id != 8) ? 'Thứ '.$v_detail->day_id : 'Chủ nhật' !!}
                                        ({{ BatvHelper::formatDate($v_detail->time_day,'Y-m-d',$formatDate='d-m-Y',$timeFormat='H:i:s',$time=false) }})
                                    </td>
                                    <td class="text-center">
                                        {{ $v_detail->hour }}
                                    </td>
                                    <td style="text-align: justify; ">
                                        @if (strlen($v_detail->content_report) >= 290)
                                            {!! BatvHelper::smartStr(nl2br($v_detail->content_report) , 290, ' (...)') !!} 
                                            <a href="javascript:void(0)" data-trigger="focus" data-toggle="popover"  data-placement="top" data-html="true"  data-content='{!! nl2br($v_detail->content_report) !!}' style="float:right; font-style: italic;font-size:12px">Xem thêm</a>
                                        @else
                                            {!! nl2br($v_detail->content_report) !!}
                                        @endif
                                    </td>
                                    <td>
                                        <div class="progress">
                                            <div class="progress-bar progress-bar-success progress-bar-striped" role="progressbar"
                                            aria-valuenow="{{ $v_detail->progress }}" aria-valuemin="0" aria-valuemax="100" style="width:{{ $v_detail->progress }}%">
                                            {{ $v_detail->progress }}%
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        ...
                                        @elseif($v_detail->score == 1)
                                            <div class="status-1">Đã đồng ý</div>
                                        @elseif($v_detail->score == 2)
                                            <div class="status-2">Đã từ chối</div>
                                        @elseif($v_detail->score == 3)
                                            <div class="status-3">Chờ duyệt</div>
                                        @endif
                                    </td>
                                    <td class="text-center">
                                        @if ($v_detail->score == 3)
                                            <button type="button" class="detail-info-overtime btn btn-xs btn-primary" data-full-name="{{ $value->fullname }}" data-score="{{ $v_detail->score }}" data-time="{!! ($v_detail->day_id != 8) ? 'Thứ '.$v_detail->day_id : 'Chủ nhật' !!} ({{ BatvHelper::formatDate($v_detail->time_day,'Y-m-d',$formatDate='d-m-Y',$timeFormat='H:i:s',$time=false) }})" data-over-time-id="{{ $value->id }}" data-day-id="{{ $v_detail->day_id }}" data-content="{{ $v_detail->content_report }}" data-content-manager="{{ $v_detail->comment_manager }}" data-hour="{{ $v_detail->hour }}" data-progress="{{ $v_detail->progress }}">
                                                Phê duyệt
                                            </button>
                                        @endif
                                    </td>
                ...
                <div class="text-center">
                    {{ count($list) > 0 ? $list->appends(Request::all())->links() : '' }} 
                </div>
            @endif
    ...
    <script>
        ...
        function actionManager(){
            ...
            $.ajax({
                method: "POST",
                url: '{{ url("toh_hrm/api/manager-report-overtime") }}',
                data:data, 
                dataType: 'json',
                // beforeSend: function() {
