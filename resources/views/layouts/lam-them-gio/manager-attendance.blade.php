            ...
            <table class="table" style="margin-bottom: 5px;">
                ...
                <tbody>
                    ...
                    @foreach ($list as $key => $value)
                        @foreach ( $value['over_time_id'] as $k => $v)
                            @foreach ( $v['info'] as $k_day => $v_day)
                                <tr>
                                    @if ($tmp == 0)
                                    <td rowspan="{{ $value['count_info'] }}"> {!! $value['fullname'] !!}</td>
                                    @endif
                                    
                                    @if ($k_day == 0)
                                    <td rowspan="{{ count($v['info']) }}">Tuần từ {{ BatvHelper::formatDate($v['apply_from'],"Y-m-d", $formatDate="d-m-Y",$timeFormat="H:i:s",false) }} đến {{ BatvHelper::formatDate($v['apply_to'],"Y-m-d", $formatDate="d-m-Y",$timeFormat="H:i:s",false) }}</td>
                                    @endif

                                    <td>
                                        {!! ($v_day[4]!= 8) ? 'Thứ '.$v_day[4] : 'Chủ nhật' !!} ({{ BatvHelper::formatDate($v_day[6],"Y-m-d", $formatDate="d-m-Y",$timeFormat="H:i:s",false) }})
                                    </td>
                    ...
                    <tr style="background: rgba(255, 0, 0, 0.56);">
                        ...
                        <td class="text-center">
                            <b style="font-size:16px">{{ $total_hour }}</b> 
                        </td>
                    </tr>
                </tbody>
            </table>
