...
@section('content')
        ...
        <div class="col-lg-10">
            <h4 class="title-fuction">
                ...
                <div class="pull-right" style="position: relative;bottom:5px;right:10px">
                    @if ($status_pending == 'Hiện tại bạn không có khoản vay mới nào.' && !$detail_loan_capital)
            ...
            @if ($detail_loan_capital)
                <table class="table table-bordered table-responsive">
                    ...
                    <tbody class="special">
                        ...
                        <td class="text-center">{{ number_format($detail_loan_capital[0]->remaining_principal + $total_interest_1 + $total_interest_incurred_1 - $total_paid_money_1) }}</td>
                    </tbody>
                </table>
                    ...
                    <tbody>
                        ...
                        @foreach ($detail_loan_capital as $key => $item)
                            <tr @if ($item->status == 1) style="background:#449d44;color:#fff" @endif>
                                ...
                                <td class="text-center">{{ number_format($item->remaining_principal) }}</td>
                                ...
                                <td class="text-center">
                                    @if ($item->month > 0 && $item->status == 0 && $flag && (($final_settlement == 0 || $final_settlement == 2 ) && ($partial_settlement == 0 || $partial_settlement == 2)))
                                        @if ($detail_loan_capital[$key]->type == 0)
                                            <button type="button" class="btn btn-xs btn-primary" onclick="remindPayMonthNowByUser({{ $item->id }})">Trả bây giờ</button>
                                        @else
                ...
                <script>
                    ...
                    function remindPayAllNowByUser(){
                        ...
                        $.ajax({
                            method: "POST",
                            url: '{{ route("remind-pay-all-now-by-user") }}',
                            data:data, 
                            dataType: 'json',
                            beforeSend: function() {
                                $(".ajax_waiting").addClass("loading");
                    ...
                    function remindPayPartialSettlementByUser(){
                        Swal.fire({
                            ...
                            if(result.value){ 
                                ...
                                $.ajax({
                                    method: "POST",
                                    url: '{{ route("remind-pay-partial-settlement-by-user") }}',
                                    data:data, 
                                    dataType: 'json',
                                    beforeSend: function() {
                                        $(".ajax_waiting").addClass("loading");
                    ...
                    function remindPayMonthNowByUser(id_history_pay_loan_capital){
                        Swal.fire({
                            ...
                            if(result.value){ 
                                ...
                                $.ajax({
                                    method: "POST",
                                    url: '{{ route("remind-pay-month-now-by-user") }}',
                                    data:data, 
                                    dataType: 'json',
                                    beforeSend: function() {
                                        $(".ajax_waiting").addClass("loading");
            ...
            @else
                <table class="table table-bordered">
                    ...
                    <tbody class="special">
                        @if ($config_loan_capital)
                            <tr>
                                <td class="text-center">
                                    Từ {{ BatvHelper::formatDate($config_loan_capital->apply_from,'Y-m-d',$formatDate='d/m/Y',$timeFormat='H:i:s',$time=false) }} 
                                    đến {{ BatvHelper::formatDate($config_loan_capital->apply_to,'Y-m-d',$formatDate='d/m/Y',$timeFormat='H:i:s',$time=false) }} 
                                </td>
                                <td class="text-center">{{ $config_loan_capital->interest_rate }}</td>
                                <td class="text-center">{{ $config_loan_capital->preferential_interest_rate }}</td>
                                <td class="text-center">{{ $config_loan_capital->count_month_preferential }}</td>
                                <td class="text-center">{{ $config_loan_capital->deferred_interest }}</td>
                                <td class="text-center">{{ $config_loan_capital->interest_file_late }}</td>
                                <td class="text-center">{{ $config_loan_capital->month_time_max }}</td>
                                <td class="text-center">{{ $config_loan_capital->x_salary }}</td>
                                <td class="text-center">{{ $config_loan_capital->time_complete_file }}</td>
                                <td class="text-center">{{ $config_loan_capital->start_month_pay }}</td>
                                <td class="text-center">
                                    <style>
                                        .popover {
                                            width:400px;
                                    ...
                                    <a href="javascript:void(0)" data-toggle="popover" data-trigger="focus" data-placement="top" data-html="true"  data-content='<table class="table table-bordered"><thead><tr> <th class="text-center">Điểm thâm niên</th> <th class="text-center">Điểm chức danh</th> <th class="text-center">Khác</th> </tr> </thead> <tbody> <tr class="text-center"><td>{{ $personnel->score_seniority }}</td><td>{{ $personnel->score_position }}</td><td>{{ $personnel->score_faith - $personnel->score_seniority - $personnel->score_position }}</td></tr></tbody></table>'>{{ $score_faith }}</a>
                                </td>
                                <td class="text-center">{{ $config_loan_capital->score_min }}</td>
                            </tr>
                        @endif
                    </tbody>
                </table>

                @if (count($history_loan_capital) > 0)
                    <h4 class="title-fuction">
                        Lịch sử vay
                    </h4>
                    <table class="table table-bordered table-responsive">
                        ...
                        @foreach ($history_loan_capital as $history)
                        <tbody>
                            <td class="text-center">{{ number_format($history->max_money) }}</td>
                            <td class="text-center">{{ BatvHelper::formatDate($history->disbursement_date,'Y-m-d',$formatDate='d/m/Y',$timeFormat='H:i:s',false) }}</td>
                            <td class="text-center">{{ BatvHelper::formatDate($history->repayment_period,'Y-m-d',$formatDate='d/m/Y',$timeFormat='H:i:s',false) }}</td>
                            <td class="text-center">{{ BatvHelper::formatDate($history->received_date,'Y-m-d',$formatDate='d/m/Y',$timeFormat='H:i:s',false) }}</td>
                            <td class="text-center">{{ $history->interest_rate }}</td>
                            <td class="text-center">{{ $history->preferential_interest_rate }}</td>
                            <td class="text-center">{{ $history->count_month_preferential }}</td>
                            <td class="text-center">{{ $history->deferred_interest }}</td>
                            <td class="text-center">{{ number_format($history->total_interest ) }}</td>
                            <td class="text-center">{{ number_format($history->total_interest_incurred ) }}</td>
                            <td class="text-center">{{ number_format($history->total_paid_money ) }}</td>
                            <td class="text-center">
                                {{ number_format(round(($history->max_money + $history->total_interest + $history->total_interest_incurred) - $history->total_paid_money )) }}
                            </td>
                        </tbody>
                        @endforeach
                    </table>
                ...
                @if ($status_pending != null)
                    <div style="color:#3c763d;font-size:15px;">
                        {{ $status_pending }}
                    </div>
                    @if ($status_pending == 'Bạn đang có yêu cầu vay chờ duyệt.')
                        <h4 class="title-fuction">
                            Thông tin đăng ký
                        </h4>
                        @if ($updated_file > 0)
                            <div class="row" id="editLoanRegister">
                                <div class="col-sm-3 form-group">
                                    ...
                                    <input data-type="currency" class="form-control input-sm" name="money" type="text" value="{{ number_format($loan_capital_pending->max_money) }}">
                                </div>
                                <div class="col-sm-3 form-group">
                                    <label>T/g vay (tháng)</label>
                                    <input class="form-control input-sm" name="month_time" type="text" value="{{ $loan_capital_pending->month_time }}" maxlength="3"  onkeyup="this.value=this.value.replace(/[^\d]/,'')">
                                </div>
                                <div class="col-sm-3 form-group">
                                    <label>T/g mong muốn giải ngân</label>
                                    <input type="text" class="form-control input-sm" name="disbursement_date_by_user" pattern="\d{1,2}/\d{1,2}/\d{4}" value="{{ BatvHelper::formatDate($loan_capital_pending->disbursement_date_by_user, 'Y-m-d', 'd/m/Y', 'H:i:s', false) }}" autocomplete="off">
                                    <script>
                                        $(function() {
                                ...
                                <div class="col-sm-3 form-group">
                                    ...
                                    <div>
                                        {{ $score_faith }}/{{ $config_loan_capital->score_min }}
                                        @if ($score_faith >= $config_loan_capital->score_min)
                                ...
                                <div class="col-sm-4 form-group">
                                    ...
                                    <div class="">
                                        <input type="radio" name="loan_purpose" value="1" @if ($loan_capital_pending->loan_purpose == 1) checked @endif> Vay mua nhà
                                    </div>
                                    <div class="">
                                        <input type="radio" name="loan_purpose" value="2" @if ($loan_capital_pending->loan_purpose == 2) checked @endif> Vay mua xe
                                    </div>
                                    <div class="loan-purpose">
                                        <input type="radio" name="loan_purpose" value="3" @if ($loan_capital_pending->loan_purpose == 3) checked @endif> Mục đích khác
                                        @if ($loan_capital_pending->loan_purpose == 3) 
                                            <textarea class="form-control" data-autoresize="" rows="5" placeholder="Nêu mục đích..." name="another_purpose">{{ $loan_capital_pending->another_purpose }}</textarea>
                                        @else
                                ...
                                <div class="col-sm-4 form-group">
                                    ...
                                    <div>
                                        <input type="radio" name="disbursement_form" @if ($loan_capital_pending->disbursement_form == 1) checked @endif value="1"> Giải ngân trực tiếp tới đơn vị thụ hưởng
                                    </div>
                                    <div class="form-group">
                                        <input type="radio" name="disbursement_form" @if ($loan_capital_pending->disbursement_form == 2) checked @endif value="2"> Giải ngân tới người vay
                                    </div>
                                    <textarea class="form-control" data-autoresize="" rows="5" placeholder="Thông tin tài khoản nhận giải ngân..." name="info_receive_disbursement">{{ $loan_capital_pending->info_receive_disbursement }}</textarea>
                                </div>
                                <div class="col-sm-4 form-group">
                                    <label>Hình thức trả</label>
                                    <div>
                                        <input type="radio" name="pay" value="1" @if ($loan_capital_pending->pay == 1) checked @endif> Trừ vào lương
                                    </div>
                                    {{-- <div>
                                        <input type="radio" name="pay" value="2" @if ($loan_capital_pending->pay == 2) checked @endif> Tự trả qua chuyển khoản
                                    </div> --}}
                                </div>
                                @if ($loan_capital_pending->status_file == 0)
                                <div class="col-sm-12 form-group">
                                    <label>Hồ sơ</label>
                            ...
                            <script>
                                            ...
                                            $.each(data, function(key,value){
                                                if (value) {
                                                    ...
                                                    if (ext != "png" && ext != "jpeg" && ext != "jpg" && ext != "gif" && ext != "webp") {
                                                        var image_path = "{{ url('/images/general/document.png') }}";
                                                    } else {
                                                        var image_path = "{{ url('/images') }}" + "/" +value;
                                                    }
                                ...
                                function editLoanRegister(){
                                    ...
                                    $.ajax({
                                        method: "POST",
                                        url: '{{ url("toh_hrm/api/user-edit-register-loan-capital") }}',
                                        data:data, 
                                        dataType: 'json',
                                        beforeSend: function() {
                                            $(".ajax_waiting").addClass("loading");
                        ...
                        @else
                            <div class="row">
                                <div class="col-sm-3 form-group">
                                    <label>Số tiền vay (VND)</label>
                                    <div>{{ number_format($loan_capital_pending->max_money) }}</div>
                                </div>
                                <div class="col-sm-3 form-group">
                                    <label>T/g vay (tháng)</label>
                                    <div>{{ $loan_capital_pending->month_time }}</div>
                                </div>
                                <div class="col-sm-3 form-group">
                                    <label>T/g mong muốn giải ngân</label>
                                    <div>{{ BatvHelper::formatDate($loan_capital_pending->disbursement_date_by_user, 'Y-m-d', 'd/m/Y', 'H:i:s', false) }}</div>
                                </div>
                                <div class="col-sm-3 form-group">
                                    <label>Điểm tín nhiệm</label>
                                    <div>
                                ...
                                <div class="col-sm-3 form-group">
                                    <label>Mục đích vay</label><br>
                                    @if ($loan_capital_pending->loan_purpose == 1)
                                        <div>Vay mua nhà</div>
                                    @elseif($loan_capital_pending->loan_purpose == 2)
                                        <div>Vay mua xe</div>
                                    @else
                                        <div>{{ $loan_capital_pending->another_purpose }}</div>
                                    @endif
                                </div>
                                <div class="col-sm-3 form-group">
                                    <label>Hình thức giải ngân</label>
                                    @if ($loan_capital_pending->disbursement_form == 1)
                                        <div>Giải ngân trực tiếp tới đơn vị thụ hưởng</div>
                                    @else
                                        <div>Giải ngân tới người vay</div>
                                    @endif

                                    @if ($loan_capital_pending->info_receive_disbursement != '')
                                        <label class="control-label">Thông tin tài khoản nhận giải ngân:</label>
                                        <div>
                                            {!! $loan_capital_pending->info_receive_disbursement !!}
                                        </div>
                                    @endif
                                    ...
                                    <div class="modal-body">
                                        <div class="row">
                                            <div class="col-sm-3 form-group">
                                                ...
                                                ?>
                                                <input data-type="currency" class="form-control input-sm" name="money" type="text" value="" placeholder="Tối đa: {{ $max_money_placeholder }} VNĐ">
                                            </div>
                                            <div class="col-sm-3 form-group">
                                                <label>T/g vay (tháng)</label>
                                                <input class="form-control input-sm" name="month_time" type="text" maxlength="3" onkeyup="this.value=this.value.replace(/[^\d]/,'')" placeholder="Tối đa: {{ $month_time_max }} tháng">
                                            </div>
                                            <div class="col-sm-3 form-group">
                                                <label>T/g mong muốn giải ngân</label>
                        ...
                        <script>
                            $(document).ready(function(){
                                ...
                                { 
                                    ...
                                    // acceptedFiles: ".png, .jpeg, .jpg, .gif",
                                    previewTemplate: document.querySelector('#preview-template').innerHTML,
                                    url: "{{ route('droponejs-file') }}" + "?register=1",
                                    headers: {
                                        'X-CSRF-TOKEN': "{{ csrf_token() }}"
                                    },
                            ...
                            function loanRegister(){
                                ...
                                $.ajax({
                                    method: "POST",
                                    url: '{{ url("toh_hrm/api/user-register-loan-capital") }}',
                                    data:data, 
                                    dataType: 'json',
                                    beforeSend: function() {
                                        $(".ajax_waiting").addClass("loading");
    ...
    ?>
    @if ($config_loan_capital_current)
        <?php
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col-sm-4 form-group">
                            <label>L/s (% năm)</label>
                            <input class="form-control input-sm" maxlength="4" name="interest_rate" type="text" value="{{ $interest_rate }}" onkeyup="this.value=this.value.replace(/[^0-9\.]/g,'')">
                        </div>
                        <div class="col-sm-4 form-group">
                            <label>L/s ưu đãi (% năm)</label>
                            <input class="form-control input-sm" maxlength="4" name="preferential_interest_rate" type="text" value="{{ $preferential_interest_rate }}" onkeyup="this.value=this.value.replace(/[^0-9\.]/g,'')">
                        </div>
                        <div class="col-sm-4 form-group">
                            <label>T/g ưu đãi (từ lúc bắt đầu trả nợ) (tháng)</label>
                            <input class="form-control input-sm" maxlength="3"   name="count_month_preferential" value="{{ $count_month_preferential }}" onkeyup="this.value=this.value.replace(/[^\d]/,'')">
                        </div>
                        <div class="col-sm-3 form-group">
                            <label>Số tiền vay (VND)</label>
    ...
    <script>
        ...
        $('#loanEstimate').on('hidden.bs.modal', function (e) {
            ...
            $('#loanEstimate input[name=interest_rate]').val('{{ $interest_rate }}');
            $('#loanEstimate input[name=preferential_interest_rate]').val('{{ $preferential_interest_rate }}');
            $('#loanEstimate input[name=count_month_preferential]').val('{{ $count_month_preferential }}');
            $('#listRepayment').html('');
        })
        start_month_pay = 2;
        ...
        $("#date").change(function(){
            ...
            if (disbursement_date != '') {
                if (validationDate(disbursement_date)) {
                    ...
                    $.ajax({
                        method: "POST",
                        url: '{{ url("toh_hrm/api/info-interest-rate-config") }}',
                        data:data, 
                        dataType: 'json',
                        beforeSend: function() {
                            $(".ajax_waiting").addClass("loading");
        ...
        function calcLoan() {
            ...
            $.ajax({
                method: "POST",
                url: '{{ url("toh_hrm/api/calculate-demo-loan-capital") }}',
                data:data, 
                dataType: 'json',
                beforeSend: function() {
                    $(".ajax_waiting").addClass("loading");
