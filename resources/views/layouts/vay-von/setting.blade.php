            ...
            <div class="tab-content">
                        ...
                        <tbody>
                            <tr>
                                <td class="text-center">
                                    {{ number_format($config_fund_loan_capital->value) }}
                                    <a href="javascript:void(0)" data-toggle="modal" data-target="#editFundLoanCapital">
                                        <img src="{{ asset('images/general/edit.png') }}">
                                    </a>
                                </td class="text-center">
                                <td class="text-center">{{ number_format($disbursement_money) }}</td>
                                <td class="text-center">{{ number_format($amount_collected) }}</td>
                                <td class="text-center">{{ number_format($interest_and_interest_incurred) }}</td>
                                <td class="text-center">{{ number_format($config_fund_loan_capital->value + $amount_collected - $disbursement_money) }}</td>
                                <td class="text-center">{{ number_format($config_fund_loan_capital->value + $amount_collected - $disbursement_money + $interest_and_interest_incurred) }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                <div id="menu2" class="tab-pane fade">
                    <table class="table table-striped">
                        ...
                        <tbody>
                            @foreach ($config_loan_capital as $value)
                                <tr>
                                    <td class="text-center">
                                        Từ {{ BatvHelper::formatDate($value->apply_from,'Y-m-d',$formatDate='d/m/Y',$timeFormat='H:i:s',$time=false) }} 
                                        đến {{ BatvHelper::formatDate($value->apply_to,'Y-m-d',$formatDate='d/m/Y',$timeFormat='H:i:s',$time=false) }} 
                                    </td>
                                    <td class="text-center">{{ $value->interest_rate }}</td>
                                    <td class="text-center">{{ $value->preferential_interest_rate }}</td>
                                    <td class="text-center">{{ $value->count_month_preferential }}</td>
                                    <td class="text-center">{{ $value->deferred_interest }}</td>
                                    <td class="text-center">{{ $value->start_month_pay }}</td>
                                    <td class="text-center">{{ $value->month_time_max }}</td>
                                    <td class="text-center">{{ $value->x_salary }}</td>
                                    <td class="text-center">{{ $value->time_complete_file }}</td>
                                    <td class="text-center">{{ $value->interest_file_late }}</td>
                                    <td class="text-center">{{ $value->score_min }}</td>
                                    <td class="text-center">
                                        <a href="javascript:void(0)" data-id="{{ $value->id }}" data-toggle="modal" data-target="#updateInterestRateConfig" class="update-interest-rate-config">
                                            <img src="{{ asset('images/general/edit.png') }}">
                                        </a>
                                        <a class="btn-delete" href="javascript:void(0)" onclick="deleteInterestRateConfig({{ $value->id }})"> 
                                            <img src="{{ asset('images/general/remove.png') }}">
                                        </a>
                                    </td>
                                </tr>
                            @endforeach
                    ...
                    <table class="table table-striped">
                        <thead>
                            <tr>
                                ...
                                {{-- <th class="text-center">Điểm tín nhiệm tối thiểu</th> --}}
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td class="text-center">{{ $config_score_tn }}</td>
                                <td class="text-center">{{ $config_score_tsp_tld }}</td>
                                <td class="text-center">{{ $config_score_tp_ptt }}</td>
                                <td class="text-center">{{ $config_score_ttt }}</td>
                                {{-- <td class="text-center">{{ $config_score_min }}</td> --}}
                                <td class="text-center">
                                    <a href="javascript:void(0)" data-id="{{ $value->id }}" data-toggle="modal" data-target="#updateScoreFaithConfig">
                                        <img src="{{ asset('images/general/edit.png') }}">
                                    </a>
                                </td>
                            </tr>
                ...
                <div id="menu4" class="tab-pane fade">
                    <table class="table table-striped">
                        ...
                        <tbody>
                            <tr>
                                <td class="text-center">{{ $config_remind_after_x_days }}</td>
                                <td class="text-center">{{ $config_remind_before_x_days }}</td>
                                <td class="text-center">{{ $config_email_tq }}</td>
                                <td class="text-center">
                                    <a href="javascript:void(0)" data-id="{{ $value->id }}" data-toggle="modal" data-target="#updateOtherConfig">
                                        <img src="{{ asset('images/general/edit.png') }}">
                                    </a>
                                </td>
                            </tr>
                    ...
                    <div class="row">
                        <div class="col-sm-5">
                            <div class="form-group clearfix">
                                ...
                                <input data-type="currency" class="form-control input-sm" name="fund_money" type="text" value="{{ BatvHelper::formatPriceSpecial($config_fund_loan_capital->value) }}">
                            </div>
                        </div>
                    </div>
    ...
    <script>
        $(document).ready(function(){
            $('#editFundLoanCapital').on('hidden.bs.modal', function () {
                $('#editFundLoanCapital [name=fund_money]').val('{{ BatvHelper::formatPriceSpecial($config_fund_loan_capital->value) }}');
            });
            ...
            $('#updateOtherConfig').on('show.bs.modal', function () {
                $('#updateOtherConfig input[name=remind_before_x_days]').val({{ $config_remind_before_x_days }});
                $('#updateOtherConfig input[name=remind_after_x_days]').val({{ $config_remind_after_x_days }});
                $('#updateOtherConfig input[name=email_tq]').val("{{ $config_email_tq }}");
            });

            $('#updateScoreFaithConfig').on('show.bs.modal', function () {
                $('#updateScoreFaithConfig input[name=score_tn]').val({{ $config_score_tn }});
                $('#updateScoreFaithConfig input[name=score_tsp_tld]').val({{ $config_score_tsp_tld }});
                $('#updateScoreFaithConfig input[name=score_tp_ptt]').val({{ $config_score_tp_ptt }});
                $('#updateScoreFaithConfig input[name=score_ttt]').val({{ $config_score_ttt }});
            });

            $('#updateInterestRateConfig').on('show.bs.modal', function () {
                $('#updateInterestRateConfig input[name=interest_rate]').val({{ $config_fund_loan_capital->interest_rate }});
            });
        ...
        $(document).on('click','.update-interest-rate-config',function(){
            ...
            $.ajax({
                method: "POST",
                url: '{{ url("toh_hrm/api/check-interest-rate-config") }}',
                data:data, 
                dataType: 'json',
                // beforeSend: function() {
        ...
        function updateFundLoanCapital(){
            ...
            $.ajax({
                method: "POST",
                url: '{{ url("toh_hrm/api/update-fund-loan-capital") }}',
                data:data, 
                dataType: 'json',
                // beforeSend: function() {
                //     $("#pre_ajax_loading").show();
        ...
        function updateLoanCapital(){
            ...
            $.ajax({
                method: "POST",
                url: '{{ url("toh_hrm/api/update-loan-capital") }}',
                data:data, 
                dataType: 'json',
                // beforeSend: function() {
                //     $("#pre_ajax_loading").show();
        ...
        function insertInterestRateConfig(){
            ...
            $.ajax({
                method: "POST",
                url: '{{ url("toh_hrm/api/insert-interest-rate-config") }}',
                data:data, 
                dataType: 'json',
                // beforeSend: function() {
                //     $("#pre_ajax_loading").show();
        ...
        function updateInterestRateConfig(){
            ...
            $.ajax({
                method: "POST",
                url: '{{ url("toh_hrm/api/update-interest-rate-config") }}',
                data:data, 
                dataType: 'json',
                // beforeSend: function() {
                //     $("#pre_ajax_loading").show();
            ...
            Swal.fire({
                ...
                cancelButtonText: 'Không'
                    ...
                    if (result.value) {
                        ...
                        $.ajax({
                            method: "POST",
                            url: '{{ url("toh_hrm/api/delete-interest-rate-config") }}',
                            data:data, 
                            dataType: 'json',
                            // beforeSend: function() {
                            //     $("#pre_ajax_loading").show();
        ...
        function updateScoreFaithConfig(){
            ...
            $.ajax({
                method: "POST",
                url: '{{ url("toh_hrm/api/update-score-faith-config") }}',
                data:data, 
                dataType: 'json',
                // beforeSend: function() {
                //     $("#pre_ajax_loading").show();
        ...
        function updateOtherConfig(){
            ...
            $.ajax({
                method: "POST",
                url: '{{ url("toh_hrm/api/update-other-config") }}',
                data:data, 
                dataType: 'json',
                // beforeSend: function() {
                //     $("#pre_ajax_loading").show();
