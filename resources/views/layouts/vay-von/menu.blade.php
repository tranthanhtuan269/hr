...
<p><a href="{{ url('toh_hrm/vay-von/index') }}"><i class="fa fa-angle-double-right" aria-hidden="true"></i> Tín dụng</a></p>
...
<script>
    ...
    $(document).on("click",".dz-image img",function() {
        window.open('{{ url("images") }}' + '/' + $(this).attr('alt'), '_blank');
    });
...
@if (isset($time_complete_file))
    <p>
        ...
        @if ($number_day_rest != '')
            (còn {{ $number_day_rest }} ngày để hoàn tất)
        @endif
    </p>
        ...
        <script>
                        ...
                        $.each(data, function(key,value){
                            if (value) {
                                ...
                                if (ext != "png" && ext != "jpeg" && ext != "jpg" && ext != "gif" && ext != "webp") {
                                    var image_path = "{{ asset('images/general/document.png') }}";
                                } else {
            ...
            function loanCompleteFile(){
                ...
                $.ajax({
                    method: "POST",
                    url: '{{ url("toh_hrm/api/loan-complete-file") }}',
                    data:data, 
                    dataType: 'json',
                    beforeSend: function() {
...
@if(in_array('vay-von-quan-ly',$arr_route))
    <p><a href="{{ url('toh_hrm/vay-von/quan-ly?selectDepart=0&status=-1') }}"><i class="fa fa-angle-double-right" aria-hidden="true"></i> Quản lý</a></p>
@endif

@if(Auth::user()->id == 1)
    <p><a href="{{ url('toh_hrm/vay-von/diem-tin-nhiem') }}"><i class="fa fa-angle-double-right" aria-hidden="true"></i> Danh sách điểm tín nhiệm</a></p>
@endif

{{-- <p><a href="{{ url('toh_hrm/vay-von/quan-ly-ho-so') }}"><i class="fa fa-angle-double-right" aria-hidden="true"></i> Phê duyệt hồ sơ</a></p> --}}
@if(in_array('vay-von-cau-hinh',$arr_route))
    <p><a href="{{ url('toh_hrm/vay-von/cau-hinh') }}"><i class="fa fa-angle-double-right" aria-hidden="true"></i> Cấu hình</a></p>
@endif
