        ...
        <style type="text/css">
            ...
            */
            .login-form input[type='submit']{
                ...
                border: none;
                margin-top: 25px;
                background: url({{ asset('images/login/dang_nhap.png') }}) ;    
            }
    ...
    <body>
        ...
        <div class="container" id="info">
            <div class="form">
                ...
                <div class="logo">
                    <image src="{{ asset('images/login/logo.png') }}" alt="TOH SOFT">
                </div>
                <h3>Công ty phần mềm Tower Hà Nội</h3>
                <h2>Hệ thống quản lý nhân sự </h2>
                <form class="login-form" method="post" action="{{url('login')}}" >
                    @if($errors->has('errorLogin'))
                    <p style="color:red;top: 5px;">{{$errors->first('errorLogin')}}</p>
                    @endif
                    <div class="row">
                        <input type="email" name="email" placeholder="Email" value="{{old('email')}}" />
                        @if($errors->has('email'))
                        <span style="color:red;padding-top: 5px; text-align:left">{{$errors->first('email')}}</span>
                        @endif
                    </div>
                    <div class="row">
                        <input type="password" name="password" placeholder="Mật khẩu"/>
                        @if($errors->has('password'))
                        <span style="color:red;padding-top: 5px; text-align:left">{{$errors->first('password')}}</span>
                        @endif
                    </div>
                    <div class="row">
