            ...
            <div class="row">
              <div class="col-lg-6">
                <div class="form-group">
                  ...
                  <div class="col-sm-8">
                    <input type="text" class="form-control" name="name" placeholder="Họ tên" value="{{ Request::get('name') }}">
                  </div>
                </div>
              </div>
              <div class="col-lg-6">
      ...
      <div class="col-lg-12">
        <h4 class="title-fuction">Danh sách người dùng 
            @if(in_array('user-add',$arr_route))
              <a href="{{ route('getUserAdd')}}"><img src="{{ asset('images/general/add.png') }}"></a>
            @endif
        </h4>
          ...
          <table class="table table-hover">
            <tbody>
              ...
              <tr>
                ...
                <td>{{ $val->email }}</td>
                <td>{!! nl2br($val->roles_name) !!}</td>
                <td>
                  @if(in_array('user-edit',$arr_route))
                    <a class="btn-edit" href="{{ route('getUserEdit',['id'=>$val->id ]) }}"><img src="{{ asset('images/general/edit.png') }}"></a>
                  @endif
                  @if(in_array('user-del',$arr_route))
                    <a class="btn-delete" href="{{ route('getUserDel',['id'=>$val->id]) }}" onclick="return confirm('Bạn có chắc chắn muốn xóa ?')"> <img src="{{ asset('images/general/remove.png') }}"></a>
                  @endif
                </td>  
              </tr>
              @endforeach
