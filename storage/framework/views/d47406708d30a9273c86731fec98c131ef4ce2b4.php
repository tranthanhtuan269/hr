			...
			<div class="col-lg-offset-2 col-lg-8">
		        <form  class="form-horizontal" method="POST">
		          <div class="form-group">
		            ...
		            <div class="col-sm-8">
		              <input type="text" class="form-control" name="title"  value="<?php echo e(old('title',isset($data[$id]['title']) ? $data[$id]['title'] : null )); ?>" required="required">
		            </div>
		          </div>
		          <div class="form-group">
	                        ...
	                        <?php if(!empty($listPersonnel)): ?>
	                            <select name="manager_id" id="my-select" >
	                                <?php foreach($listPersonnel as $val): ?>
	                                     <option value="<?php echo e($val->id); ?>" <?php echo ( $val->id==$data[$id]['manager_id'])?"selected":""; ?> ><?php echo e($val->fullname); ?></option>
	                                <?php endforeach; ?>
	                            </select>
	                        <?php endif; ?>
						...
						<div class="col-sm-8">	
	                        <?php if(!empty($listPersonnel)): ?>
	                            <select id="my-select-2" name="personnel_attendance[]" multiple="multiple">
	                                <?php foreach($listPersonnel as $val): ?>
	                                     <option value="<?php echo e($val->id); ?>" <?php echo ( in_array($val->id,$data[$id]['manage_id_attendance']) )?"selected":""; ?>><?php echo e($val->fullname); ?></option>data
	                                <?php endforeach; ?>
	                            </select>
	                        <?php endif; ?>
							<script type="text/javascript">
