...
<?php $__env->startSection('title', 'Chấm công đi muộn'); ?>
				...
				<div class="form-group col-lg-12">
					...
					<div class="col-sm-9">
						<select name="filter" class="form-control">
			                <option value="0" <?php if(Input::get('filter') == '0'): ?> selected="selected" <?php endif; ?>> Chọn </option>
			                <option value="1" <?php if(Input::get('filter') == '1'): ?> selected="selected" <?php endif; ?>> Danh sách không đi muộn </option>
			                <option value="2" <?php if(Input::get('filter') == '2'): ?> selected="selected" <?php endif; ?>> Danh sách đi muộn </option>
			                <option value="3" <?php if(Input::get('filter') == '3'): ?> selected="selected" <?php endif; ?>> Danh sách đi muộn vi phạm</option>
						</select>
					</div>
				</div>
				<div class="form-group col-lg-12">
        ...
        <table>
            ...
            <?php if(!empty($data)): ?>
				<?php foreach($data as $val): ?>
				   ...
				   <tr>
					<th style="<?php echo e($css_timelate_warning); ?>"><?php echo e(str_limit( $val->fullname, $limit = 30, $end = '...')); ?></th>
					<?php $total_time_late = 0; ?>
					<?php for($i = 1 ; $i <= $number ; $i++): ?>
					    <?php if(!empty($val->time_late[$i])): ?>
					    	<?php $total_time_late += $val->time_late[$i]; ?>
					        <td style="color:white;background:rgba(0, 53, 255, 0.61);font-weight: bold;"><?php echo e($val->time_late[$i]); ?></td>
						<?php else: ?>
								<td>&nbsp</td>
						<?php endif; ?>
					<?php endfor; ?>
						<td style="background:#f5f5f5;font-weight: bold;"><?php echo count( $val->time_late ); ?></td>
						<td style="background:#f5f5f5;font-weight: bold;"><?php echo e($total_time_late); ?></td>
					</tr>
				<?php endforeach; ?>
            <?php endif; ?>
